...
class Worker:
    def __init__(
        ...
        self.text_grabber_fn = text_grabber_fn if text_grabber_fn else default_text_grabber_fn

        self.data_generator_fn = data_generator if data_generator else partial(_default_text_source, worker_id=worker_id, total_workers=total_workers)
    ...
    def initialize_data_iterator(self):
        ...
        def _inf_loop(source):
            while True:
                gen = source() if callable(source) else iter(source)
                for y in gen:
                    yield y
                if not self.infinite_loop:
                    break
    ...
    def _try_get_instructions(self):
        ...
        arr = np.float32([obj.get("prob", 1.0) for obj in objs])
        s = arr.sum()
        self.og_probs = arr / s if s > 0 else np.float32([1.0 / len(objs)] * len(objs))
        return True
    ...
    def _produce_one_sample(self):
        ...
        if status in ["ready", "full"]:
            batch_data = self.batcher.pop(peek=False)
            if batch_data is not None:
                item = {"worker_id": self.worker_id, "batch_items": batch_data, "objective_name": objective_name}
                while True:
                    try:
                        self.results_queue.put(item, timeout=0.5)
                        break
